	function init() {
		    ...
		    function checkConnection() {
		        ...
		        if(states[networkState] == 0) {
		        	$('#cont').html('<h1> No Access to Internet </h1><p>A Working connection is needed...</p><button id="checkCon"> Try Connecting Again </button>');
		        }
		        else {
		        	startApp();
		        }
        ...
        setInterval("trigger()",10000);
			...
			function trigger() {
				...
				success : function(xmlcontent) {
							...
							$('#result').html('<p>updates found: '+ (currentcount-count)+ '</p>'); 
	                        //temp = "";					
